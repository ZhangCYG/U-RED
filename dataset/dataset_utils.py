...
BASE_DIR = os.path.normpath('/data/wanggu/Storage/retrieve')
import argparse
import h5py
import numpy as np
import csv
...
def render_point_cloud(point_cloud_file, point_labels_file, snapshot_file):
    g_renderer = '/home/zcyg/libigl-renderer/build/OSMesaRenderer'
    g_azimuth_deg = -70
    g_elevation_deg = 20
    g_theta_deg = 0
...
def get_source_points(source_labels, source_model_info, device):
    ...
    for source_label in source_labels:
        src_points = source_model_info[source_label]["points"]
        src_point_label = source_model_info[source_label]["point_labels"]
